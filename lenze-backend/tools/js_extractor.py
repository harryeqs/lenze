...
# Function to process a single URL using Playwright
async def process_url(url, context, semaphore, retries=3):
        ...
        for attempt in range(retries):
            try:
                ...
                if content_type == "application/pdf":
                    try:
                        pdf_buffer = await page.evaluate("() => fetch(window.location.href).then(res => res.arrayBuffer())")
                ...
                # Handle HTML content (including JS-rendered)
                try:
...
# Main function to process a list of URLs concurrently
